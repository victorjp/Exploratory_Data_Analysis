...
df %>% ggplot(aes(x=log10(residual.sugar),y=density)) + geom_point(aes(color=quality_group),alpha=0.2)

```

```{r}

df %>% ggplot(aes(x=sulphates,y=free.sulfur.dioxide)) + facet_wrap(~quality_group) + geom_point()
...
df %>% ggplot(aes(x=log2(chlorides),y=alcohol)) + facet_wrap(~quality_group) + geom_jitter()
...
df %>% ggplot(aes(x=volatile.acidity,y=alcohol,color=quality)) + geom_jitter(alpha=0.2)
