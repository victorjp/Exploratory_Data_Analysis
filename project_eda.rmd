...
central. Residual sugar is a little skewed so I will have a look on that one.
...
df %>% ggplot(aes(x=fixed.acidity)) + geom_histogram(bins = 50)
...
# it seems that the higher de quality the more uniform, could be a function of 
# sample size. There are less "good" quality wines
...
#Add something about cholorides, acidity and sulfur.
...
#add quality with chloride, pH or other stuff.
...
band of density between 0.99 g/cm^3 and 1 g/cm^3 . For the other groups
the variability seems much broader. In other words, good wines seem to have 
some specific limits of density and alcohol. Another reason of this graph is to
show the relationship between alcohol and density. The higher the alcohol content
the lower the density, which makes sense because of the lower density of alcohol itself
compared to water. 
...
df %>% ggplot(aes(x=log10(residual.sugar),y=density,color=quality_group)) +  scale_x_continuous(limits = c(0,1.5)) + scale_y_continuous(limits = c(0.985,1.005)) + ggtitle("Log10 Residual Sugar vs Density") + ylab("Density (g / cm^3)") + xlab("Log10 Residual Sugar (g / dm^3)")  + geom_smooth()

```

### Description Two
The residual sugars bimodal distribution go my attention before. This graphs
shows how the residual sugar is related to density across all wine quality
groups. However there does not seems to be a some parameters that apply only
...
df %>% ggplot(aes(x=total.sulfur.dioxide,y=log10(chlorides),color=quality_group)) + geom_jitter(alpha=0.2) 
#+ scale_color_gradientn(colours = terrain.colors(10)) + theme_dark()
#+ geom_hline(yintercept = 11,linetype= 2,alpha=0.7,color="blue")  
#+ scale_x_continuous(limits = c(0,300))
#Double check this for the final graphs. 
#+ geom_vline(xintercept = 200,linetype= 2,alpha=0.7,color="blue")

```

### Description Three
Sulfur seemed the other important variable regarding 
