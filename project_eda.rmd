...
df %>% ggplot(aes(x=log10(residual.sugar),y=density)) + geom_smooth(aes(color=quality_group)) 
...
to alcohol. There does not seems to exist a relationship between alchol and sulfur
...
but besides does limit it could vari.

```{r}

df %>% ggplot(aes(x=volatile.acidity,y=alcohol)) + facet_wrap(~quality_group) + geom_jitter()
...
df %>% ggplot(aes(x=density,y=alcohol,color=quality_group)) + geom_point(alpha=0.3) + facet_wrap(~quality_group) + scale_x_continuous(limits = c(0.99,1.005))
...
df %>% ggplot(aes(x=total.sulfur.dioxide,y=alcohol,color=quality)) + facet_wrap(~quality_group) + geom_point() + scale_color_gradientn(colours = terrain.colors(8)) + geom_hline(yintercept = 10)  + geom_vline(xintercept = 50)
